...
## Raw Content and Metadata Pairing

The NOVA RAG pipeline requires two things per document: the **raw content file** and its **enriched metadata JSON**. Every document in this corpus has both, paired 1:1 by filename stem.

### How the pairing works

```
ecfr/html/Reg_Q_Capital_Adequacy_..._12CFR217.html     <- Raw content (ingested by parser)
ecfr/json/Reg_Q_Capital_Adequacy_..._12CFR217.json     <- Enriched metadata (drives pipeline)
ecfr/md/Reg_Q_Capital_Adequacy_..._12CFR217.md         <- Readable text (for embedding/search)
ecfr/pdf/Reg_Q_Capital_Adequacy_..._12CFR217.pdf       <- Rendered PDF
```

The JSON metadata file contains **no embedded content** - it is purely metadata. The raw content lives in the HTML (or PDF) file. The MD file is the authoritative text representation derived from the HTML, used for chunking, embedding, and search indexing.

### Raw content format by source

| Source | Raw Format | How It Was Obtained | What It Contains |
|--------|-----------|---------------------|------------------|
...
| **Federal Register** | HTML | Constructed from FR API JSON response (`abstract`, `action`, links) | Document summary with abstract, action, FR/PDF links. Full text available via `html_url` and `pdf_url` |
...
### File format roles in the NOVA pipeline

| Format | Pipeline Role | When It's Used |
|--------|--------------|----------------|
| **HTML** | Raw content (bronze layer) | Input to the parser for `CanonicalDocument` creation. Contains structural markup (`<h1>`-`<h6>`, `<div class="section">`, `<table>`) that the parser uses to build `heading_path` and `section_path` |
| **PDF** | Raw content (bronze layer, alternative) | Alternative input for parsers that prefer PDF (e.g., Azure Document Intelligence). Also serves as the human-readable archival format |
| **MD** | Authoritative text (silver layer) | Used for chunking, embedding (`embed_texts()`), and BM25 indexing. This is the text that gets the Layer 1 semantic header prepended before embedding |
| **JSON** | Enriched metadata (silver layer) | Drives all three NOVA metadata layers. Fields are consumed by `semantic_header()` (Layer 1), index filters (Layer 2), and `render_hit_for_prompt()` (Layer 3) |
...
| `effective_date_start` | When it took effect | `2026-03-09` |
...
| `version_id` | Temporal version | `2026-03-09` |
| `version_label` | Human-readable version | `2026` |
...
**eCFR-specific:** `title_number`, `chapter`, `part_number`, `part_name`, `regulation_letter`, `cfr_citation`, `authority`, `section_count`, `appendix_count`, `section_headings`, `ecfr_current_as_of`, applicability flags (`applies_to_state_member_banks`, `applies_to_bank_holding_companies`, etc.)
